...
//Global Variables
char serviceName[] = "PathSolutions TFTP Service";
char displayName[] = "PathSolutions TFTP Service";
char sVersion[] = "PathSolutions TFTP Server Version 1.0";
			...
			if (GetModuleFileName(0, path, sizeof(path) / sizeof(path[0])) > 0)
			{
				...
				if (service)
				{
					...
					if (schService == NULL)
					{
						...
						char szDesc[] = "Basic TFTP server for sending and receiving files.";
...
void init(void *lpParam)
{
	...
	*fileExt = 0;
	fileExt++;
	//sprintf(logFile, "%s\\%s%%Y%%m%%d.log", extbuff, fileExt);
	//sprintf(logFile, "tftp-service.log");
	...
	do
	{
		...
		if (f = openSection("LISTEN-ON", 1, iniFile))
		{
			...
			while (readSection(raw, f))
			{
				...
				if(i < MAX_SERVERS && name[0])
				{
					ifSpecified = true;
					MYDWORD addr = my_inet_addr(name);

					if (isIP(name) && addr && (!value[0] || atoi(value)))
					{
						for (MYBYTE m = 0; ; m++)
						{
							if (m >= MAX_SERVERS || !newNetwork.allServers[m])
							{
								sprintf(logBuff, "Warning: Section [LISTEN-ON], Interface %s not available", raw);
								logMess(logBuff, 1);
								break;
							}
							else if (newNetwork.allServers[m] == addr)
							{
								for (MYBYTE n = 0; n < MAX_SERVERS; n++)
								{
									...
									{
										newNetwork.listenServers[n] = addr;

										if (value[0])
											newNetwork.listenPorts[n] = atoi(value);
										else
		...
		if (!ifSpecified)
		{
			for (MYBYTE m = 0; m < MAX_SERVERS && newNetwork.staticServers[m]; m++)
			{
				newNetwork.listenServers[m] = newNetwork.staticServers[m];
				newNetwork.listenPorts[m] = 69;
			}
		}
		...
		for (int j = 0; j < MAX_SERVERS && newNetwork.listenServers[j]; j++)
		{
			...
			{
				if (network.tftpConn[k].ready && network.tftpConn[k].server == newNetwork.listenServers[j])
					break;
			}
			...
			else
			{
				...
				if (!newNetwork.listenPorts[j])
					newNetwork.listenPorts[j] = 69;
				...
				socklen_t nRet = bind(newNetwork.tftpConn[i].sock,
									  (sockaddr*)&newNetwork.tftpConn[i].addr,
				...
				if (nRet == SOCKET_ERROR)
				{
					...
					sprintf(logBuff, "%s Port %u, bind failed", IP2String(tempbuff, newNetwork.listenServers[j]), newNetwork.listenPorts[j]);
					logMess(logBuff, 1);
					continue;
				}
...
void logMess(char* logMessage, MYBYTE logLevel) {
...
void logMess(request* req, MYBYTE logLevel) {
	...
	if (verbatim)
	{
		...
		if (req->path[0])
			printf("[%s]: Client %s:%u %s, %s\n", timeString, IP2String(tempbuff, req->client.sin_addr.s_addr), ntohs(req->client.sin_port), req->path, req->serverError.errormessage);
		else
			printf("[%s]: Client %s:%u, %s\n", timeString, IP2String(tempbuff, req->client.sin_addr.s_addr), ntohs(req->client.sin_port), req->serverError.errormessage);
	}
		...
		if (file == NULL)
			return;
		if (req->path[0])
			fprintf(file, "[%s] Client %s:%u %s, %s\n", timeString, IP2String(tempbuff, req->client.sin_addr.s_addr), ntohs(req->client.sin_port), req->path, req->serverError.errormessage);
		else
			fprintf(file, "[%s] Client %s:%u, %s\n", timeString, IP2String(tempbuff, req->client.sin_addr.s_addr), ntohs(req->client.sin_port), req->serverError.errormessage);
...
void write_registry(string g_log_path)
{
	HKEY hKey;
	char subKey[] = "HKEY_CURRENT_USER\\SOFTWARE\\PathSolutionTFTPServer";
	char valueName[] = "LogPath";

	DWORD dataSize = 256;
	WCHAR buffer[256];

	cout << "Registering log path ... " << g_log_path << endl;
	...
	if (RegCreateKeyEx(HKEY_CLASSES_ROOT, subKey, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE | KEY_READ, NULL, &hKey, NULL) == ERROR_SUCCESS) {
		...
		if (RegSetValueEx(hKey, valueName, 0, REG_SZ, (LPBYTE)g_log_path.c_str(), (DWORD)(g_log_path.length() + 1) * sizeof(char)) == ERROR_SUCCESS) {
...
void read_log_path_registry()
{
	...
	if (RegCreateKeyEx(HKEY_CLASSES_ROOT, subKey, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE | KEY_READ, NULL, &hKey, NULL) == ERROR_SUCCESS) {
		...
		if (RegQueryValueEx(hKey, valueName, NULL, NULL, NULL, &dataSize) == ERROR_SUCCESS) {
			// Resize the string to accommodate the value data
			valueData.resize(dataSize / sizeof(TCHAR));

			// Read the string value from the registry
			if (RegQueryValueEx(hKey, valueName, NULL, NULL, (LPBYTE)valueData.data(), &dataSize) == ERROR_SUCCESS) {
				std::cout << "Log path read from the registry: " << valueData << std::endl;
				strcpy(logFile, valueData.c_str());
			}
			else {
				std::cerr << "Failed to read log path from the registry." << std::endl;
			}
		}
		else {
...
"\n"
" TFTPService v1.0 Copyright 2023 PathSolutions, Inc.\n"
"\n"
"  usage:  %s <options> \n"
"\n"
"  Options:\n"
"	  -l       <path>     : Output logging information to the specific text file\n"
"	  -p                  : Start program with standalone mode\n"
"	  -i                  : Install service\n"
"	  -u                  : Uninstall service\n"
"\n"
...
void tftp_server_usage(TCHAR* argv[])
{
	printf(usage, argv[0]);
}
...
int main(int argc, TCHAR* argv[])
{
	...
	if (result && osvi.dwPlatformId >= VER_PLATFORM_WIN32_NT)
	{
		if (argc > 1 && lstrcmpi(argv[1], TEXT("-i")) == 0)
			...
			uninstallService();
		else if (argc > 1 && lstrcmpi(argv[1], TEXT("-p")) == 0)
		{
			SC_HANDLE serviceControlManager = OpenSCManager(0, 0, SC_MANAGER_CONNECT);
		...
		else if (argc > 1 && (lstrcmpi(argv[1], TEXT("-h")) == 0 || lstrcmpi(argv[1], TEXT("-help")) == 0 || lstrcmpi(argv[1], TEXT("-?")) == 0)) {
			tftp_server_usage(argv);
		}
		else if (argc > 1 && lstrcmpi(argv[1], TEXT("-l")) == 0) {
			string g_log_path;
			std::wstring_convert<std::codecvt_utf8_utf16<_TCHAR>, _TCHAR> converter;
			g_log_path = converter.to_bytes(argv[2]); // Convert to std::string
			write_registry(g_log_path);
		}
		else
			runService();
	}
